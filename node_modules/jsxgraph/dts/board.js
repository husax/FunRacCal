...
define([
...
], function (JXG, Const, Coords, Options, Numerics, Mat, Geometry, Complex, Statistics, JessieCode, GeonextParser, Color, Type, EventEmitter, Env, Transform, Point, Line, Text, Composition, EComposition) {
    'use strict';
    /**
    ...
    JXG.extend(JXG.Board.prototype, /** @lends JXG.Board.prototype */ {
        ...
        setId: function (obj, type) {
            var num = this.numObjects, elId = obj.id;
            this.numObjects += 1;
        ...
        getCoordsTopLeftCorner: function () {
            var cPos, doc, crect, docElement = this.document.documentElement || this.document.body.parentNode, docBody = this.document.body, container = this.containerObj, viewport, content;
            /**
        ...
        initMoveObject: function (x, y, evt, type) {
            var pEl, el, collect = [], offset = [], haspoint, len = this.objectsList.length, dragEl = { visProp: { layer: -10000 } };
        ...
        moveObject: function (x, y, o, evt, type) {
            var newPos = new Coords(Const.COORDS_BY_SCREEN, this.getScrCoordsOfMouse(x, y), this), drag;
            if (!(o && o.obj)) {
                return;
            }
            drag = o.obj;
            ...
            if (Type.exists(drag.coords)) {
                ...
                if (!isNaN(o.targets[0].Xprev + o.targets[0].Yprev)) {
                    drag.setPositionDirectly(Const.COORDS_BY_SCREEN, [newPos.scrCoords[1], newPos.scrCoords[2]], [o.targets[0].Xprev, o.targets[0].Yprev]);
                }
        ...
        twoFingerTouchObject: function (np1c, np2c, o, drag) {
            var np1, np2, op1, op2, nmid, omid, nd, od, d, S, alpha, t1, t2, t3, t4, t5, ar, i, len;
        ...
        twoFingerTouchCircle: function (np1c, np2c, o, drag) {
            var np1, np2, op1, op2, d, alpha, t1, t2, t3, t4, t5;
        ...
        mouseOriginMoveStart: function (evt) {
            var r = this.attr.pan.enabled && (!this.attr.pan.needshift || evt.shiftKey), pos;
            if (r) {
        ...
        touchOriginMoveStart: function (evt) {
            var touches = evt[JXG.touchProperty], twoFingersCondition = (touches.length === 2 && Geometry.distance([touches[0].screenX, touches[0].screenY], [touches[1].screenX, touches[1].screenY]) < 120), r = this.attr.pan.enabled && (!this.attr.pan.needtwofingers || twoFingersCondition), pos;
            if (r) {
        ...
        gestureChangeListener: function (evt) {
            var c, zx = this.attr.zoom.factorx, zy = this.attr.zoom.factory, dist;
        ...
        pointerDownListener: function (evt, object) {
            var i, j, k, pos, elements, sel, eps = this.options.precision.touch, found, target, result;
            if (!this.hasPointerUp) {
                if (window.navigator.pointerEnabled) { // IE11+
        ...
        touchStartListener: function (evt) {
            var i, pos, elements, j, k, time, eps = this.options.precision.touch, obj, found, targets, evtTouches = evt[JXG.touchProperty], target;
            if (!this.hasTouchEnd) {
        ...
        touchMoveListener: function (evt) {
            var i, pos1, pos2, time, evtTouches = evt[JXG.touchProperty];
            if (this.mode !== this.BOARD_MODE_NONE) {
                evt.preventDefault();
                evt.stopPropagation();
            }
        ...
        touchEndListener: function (evt) {
            var i, j, k, eps = this.options.precision.touch, tmpTouches = [], found, foundNumber, evtTouches = evt && evt[JXG.touchProperty];
        ...
        mouseWheelListener: function (evt) {
            ...
            var wd = evt.detail ? -evt.detail : evt.wheelDelta / 40, pos = new Coords(Const.COORDS_BY_SCREEN, this.getMousePosition(evt), this);
            if (wd > 0) {
        ...
        getUsrCoordsOfMouse: function (evt) {
            var cPos = this.getCoordsTopLeftCorner(), absPos = Env.getPosition(evt, null, this.document), x = absPos[0] - cPos[0], y = absPos[1] - cPos[1], newCoords = new Coords(Const.COORDS_BY_SCREEN, [x, y], this);
        ...
        addConditions: function (str) {
            var term, m, left, right, name, el, property, functions = [], plaintext = 'var el, x, y, c, rgbo;\n', i = str.indexOf('<data>'), j = str.indexOf('<' + '/data>'), xyFun = function (board, el, f, what) {
                return function () {
                    var e, t;
        ...
        calculateSnapSizes: function () {
            var p1 = new Coords(Const.COORDS_BY_USER, [0, 0], this), p2 = new Coords(Const.COORDS_BY_USER, [this.options.grid.gridX, this.options.grid.gridY], this), x = p1.scrCoords[1] - p2.scrCoords[1], y = p1.scrCoords[2] - p2.scrCoords[2];
         ...
         */
        zoomIn: function (x, y) {
            var bb = this.getBoundingBox(), zX = this.attr.zoom.factorx, zY = this.attr.zoom.factory, dX = (bb[2] - bb[0]) * (1.0 - 1.0 / zX), dY = (bb[1] - bb[3]) * (1.0 - 1.0 / zY), lr = 0.5, tr = 0.5;
        ...
        zoomOut: function (x, y) {
            var bb = this.getBoundingBox(), zX = this.attr.zoom.factorx, zY = this.attr.zoom.factory, dX = (bb[2] - bb[0]) * (1.0 - zX), dY = (bb[1] - bb[3]) * (1.0 - zY), lr = 0.5, tr = 0.5, mi = this.attr.zoom.eps || this.attr.zoom.min || 0.001; // this.attr.zoom.eps is deprecated
         ...
         */
        zoom100: function () {
            var bb = this.getBoundingBox(), dX = (bb[2] - bb[0]) * (1.0 - this.zoomX) * 0.5, dY = (bb[1] - bb[3]) * (1.0 - this.zoomY) * 0.5;
        ...
        zoomElements: function (elements) {
            var i, j, e, box, newBBox = [0, 0, 0, 0], dir = [1, -1, -1, 1];
        ...
        updateRendererCanvas: function () {
            var el, pEl, i, mini, la, olen = this.objectsList.length, layers = this.options.layer, len = this.options.layer.numlayers, last = Number.NEGATIVE_INFINITY;
            for (i = 0; i < len; i++) {
        ...
        setBoundingBox: function (bbox, keepaspectratio) {
            var h, w, dim = Env.getDimensions(this.container, this.document);
        ...
        getBoundingBox: function () {
            var ul = new Coords(Const.COORDS_BY_SCREEN, [0, 0], this), lr = new Coords(Const.COORDS_BY_SCREEN, [this.canvasWidth, this.canvasHeight], this);
        ...
        */
        ...
        createRoulette: function (c1, c2, start_c1, stepsize, direction, time, pointlist) {
            var brd = this, Roulette = function () {
                var alpha = 0, Tx = 0, Ty = 0, t1 = start_c1, t2 = Numerics.root(function (t) {
                    var c1x = c1.X(t1), c1y = c1.Y(t1), c2x = c2.X(t), c2y = c2.Y(t);
                    return (c1x - c2x) * (c1x - c2x) + (c1y - c2y) * (c1y - c2y);
                }, [0, Math.PI * 2]), t1_new = 0.0, t2_new = 0.0, c1dist, rotation = brd.create('transform', [
                    function () {
                        return alpha;
                    }
                ], { type: 'rotate' }), rotationLocal = brd.create('transform', [
                    function () {
                        return alpha;
                    },
                    function () {
                ...
                ], { type: 'rotate' }), translate = brd.create('transform', [
                    function () {
                        return Tx;
                    },
                    function () {
                ...
                arclen = function (c, a, b) {
                    var cpxa = Numerics.D(c.X)(a), cpya = Numerics.D(c.Y)(a), cpxb = Numerics.D(c.X)(b), cpyb = Numerics.D(c.Y)(b), cpxab = Numerics.D(c.X)((a + b) * 0.5), cpyab = Numerics.D(c.Y)((a + b) * 0.5), fa = Math.sqrt(cpxa * cpxa + cpya * cpya), fb = Math.sqrt(cpxb * cpxb + cpyb * cpyb), fab = Math.sqrt(cpxab * cpxab + cpyab * cpyab);
